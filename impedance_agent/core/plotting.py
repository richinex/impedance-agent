...
class PlotManager:
    ...
    def _plot_nyquist(result: AnalysisResult, ax) -> None:
        ...
        ax.plot(Z.real, -Z.imag, 'o', color='#1f77b4', label='Experimental',
                markerfacecolor='none', markeredgewidth=0.75)

        # Plot ECM fit if available with defensive check
        if (result.ecm_fit is not None and
            hasattr(result.ecm_fit, 'Z_fit') and
            result.ecm_fit.Z_fit is not None):
            z_fit = result.ecm_fit.Z_fit
            ax.plot(z_fit.real, -z_fit.imag, '-', color='#ff7f0e',
                   label='ECM Fit', linewidth=0.75)
    ...
    def _plot_drt(result: AnalysisResult, ax) -> None:
        ...
        ax.plot(freq, result.drt_fit.gamma, '-', color='#1f77b4',
                linewidth=0.75, label='DRT')

        # Mark peaks with defensive check
        if (hasattr(result.drt_fit, 'peak_frequencies') and
            result.drt_fit.peak_frequencies is not None and
            hasattr(result.drt_fit, 'peak_polarizations') and
            result.drt_fit.peak_polarizations is not None):
            ax.scatter(result.drt_fit.peak_frequencies,
                      result.drt_fit.peak_polarizations,
                      color='#ff7f0e', marker='o', s=20,
                      label='Peak Processes', zorder=5,
    ...
    def _plot_linkk(result: AnalysisResult, ax) -> None:
        ...
        if not result.drt_fit or not hasattr(result.drt_fit, 'tau'):
            PlotManager.logger.warning("No tau values available for frequency calculation")
            return

        if (not hasattr(result.linkk_fit, 'residuals_real') or
            not hasattr(result.linkk_fit, 'residuals_imag')):
        ...
        ax.plot(freq, result.linkk_fit.residuals_real * 100, 'o',
                color='#1f77b4', label='Real',
                markerfacecolor='none', markeredgewidth=0.75)
        ax.plot(freq, result.linkk_fit.residuals_imag * 100, 'o',
                color='#ff7f0e', label='Imaginary',
                markerfacecolor='none', markeredgewidth=0.75)
        ...
        ax.legend(loc='best')
        ax.set_title(f"Lin-KK Validation (M={getattr(result.linkk_fit, 'M', 'N/A')})", pad=8)
    ...
    def _plot_bode(result: AnalysisResult, ax) -> None:
        ...
        # Plot experimental data
        (p1,) = ax.semilogx(freq, magnitude, 'o', color='blue',
                           label='|Z|', markerfacecolor='none', markeredgewidth=0.75)
        (p2,) = ax2.semilogx(freq, phase, 'o', color='orange',
                            label='Phase', markerfacecolor='none', markeredgewidth=0.75)
        ...
        if (result.ecm_fit is not None and
            ...
            ax.semilogx(freq, magnitude_fit, '-', color='red',
                       label='|Z| fit', linewidth=0.75)
            ax2.semilogx(freq, phase_fit, '-', color='purple',
                        label='Phase fit', linewidth=0.75)
        ...
        ax.set_ylim([np.min(magnitude) - 0.15*magnitude_range,
        ...
        ax.legend(lines, labels, loc='best', frameon=True,
                 framealpha=0.9, edgecolor='none', fancybox=True)
